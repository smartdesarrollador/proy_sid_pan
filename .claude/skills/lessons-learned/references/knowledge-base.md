...
### LL-098 — Sesión web del Hub sigue activa → el middleware descarta `source=desktop&state=` y el deep-link SSO al Desktop nunca se dispara
- **Síntoma:** El usuario cierra sesión en `frontend_sidebar_desktop` (Tauri) y da click en "Iniciar sesión" de nuevo, pero el sidebar se queda esperando indefinidamente (el poll de `poll_deep_link_url` cada 500ms expira a los 120s sin recibir nada). En el navegador, la pestaña del Hub (`frontend_next_hub`) sigue mostrando el dashboard con sesión activa en todo momento — nunca pide credenciales.
- **Causa raíz:** `open_hub_login` (Rust, `lib.rs`) abre el **navegador del sistema** (no un webview embebido) en `${HUB_URL}/login?source=desktop&state=<nonce>`. Si el navegador ya tiene la cookie `hub-refreshToken` (sesión web activa), `middleware.ts` redirige `/login` → `/dashboard` **antes de que React monte** y **descarta todos los query params**, incluidos `source`/`state`. La lógica que construye el deep link `rbacdesktop://auth?payload=...` vivía únicamente en `LoginPageClient.tsx`, disparada solo tras un submit exitoso del formulario — que nunca ocurre porque el usuario jamás llega a ver el formulario.
- **Solución:** (1) `middleware.ts` ahora preserva `source`/`state` cuando redirige un `/login` ya autenticado, forwardeándolos a `/dashboard?source=desktop&state=...` (solo ese par, no otros query params, para no afectar flujos como `?next=`). (2) Nuevo hook `useDesktopHandoff` (`features/auth/hooks/`), consumido desde `DashboardPageClient.tsx`: detecta esos params, espera a que el restore de sesión (`AuthContext`) termine, y si el usuario ya está autenticado construye y dispara el mismo deep link `rbacdesktop://...` reusando la sesión web activa (sin pedir credenciales de nuevo); si no hay sesión válida, limpia la cookie (`clearRefreshTokenCookie`) y rebota a `/login?source=desktop&state=...` para evitar un loop de redirects. (3) El builder del payload (`{access_token, refresh_token, user, tenant}` en base64) se extrajo de `LoginPageClient.tsx` a un util compartido (`buildDesktopDeepLinkUrl.ts`) para no duplicarlo.
- **Prevención:** Cualquier redirect de middleware que descarte query params debe evaluarse contra los flujos de handoff entre apps (SSO, deep link) que dependan de ellos — un `NextResponse.redirect(new URL(path, url))` sin `.search` es fácil de escribir y silenciosamente rompe cualquier flujo que dependa del querystring sobrevivir la redirección. Cuando un flujo de auth solo se dispara "tras submit exitoso del formulario", preguntarse explícitamente: ¿qué pasa si el usuario nunca ve el formulario porque ya está autenticado? Ese caso (`isAuth && refreshToken` ya presente) necesita su propio camino de finalización del flujo, no asumir que el formulario siempre se renderiza.
- **Fuente:** sesión de fix 2026-07-14 (sin reporte dedicado en `reports/`; ver `BACKLOG.md` → "3 Últimas tareas realizadas").
- **Tags:** middleware, nextjs, deep-link, tauri, desktop, sso, rbacdesktop, query-params, redirect, session, hub

### LL-099 — `app.restart()` en `tauri dev` deja la instancia nueva sin frontend (ERR_CONNECTION_REFUSED): el CLI mata a Vite al morir el proceso original
- **Síntoma:** Tras usar el botón "Reiniciar ahora" de Configuración → Comportamiento (cambio de ubicación de la barra), el sidebar reaparece anclado al borde correcto pero mostrando solo texto blanco: es la página de error del WebView2 — "localhost rechazó la conexión / ERR_CONNECTION_REFUSED" hacia `http://localhost:1420` (el `devUrl`). Engañoso: el usuario lo reportó como "derecha rota, izquierda funciona", pero la dirección es coincidencia — el lado que "funcionaba" venía de un relanzamiento manual fresco de `tauri dev`, y el que "fallaba" de la instancia huérfana del restart.
- **Causa raíz:** El comando `restart_app` (Rust) usaba `app.restart()`. En dev, el ciclo de vida del CLI de `tauri dev` está atado al proceso de la app: `app.restart()` lanza una instancia nueva **detached** y termina la original → el CLI detecta la salida, apaga el `beforeDevCommand` (Vite) y termina → la instancia relanzada intenta cargar `devUrl` y no hay nadie escuchando. En build de producción no aplica (assets embebidos), pero el flujo de desarrollo queda roto **siempre**, sin importar qué setting se cambió.
- **Solución:** Reemplazar el restart de proceso por `window.location.reload()` en `SettingsPanel.tsx` (botón "Aplicar ahora"). Funciona porque el reload re-evalúa los módulos JS → `INITIAL_SIDEBAR_POSITION` se re-captura de localStorage → el efecto de `App.tsx` re-invoca `register_appbar` con el borde nuevo, y ese comando ya es registro fresco idempotente que además reposiciona la ventana (`ABM_REMOVE` → `ABM_NEW` → `reserve_band` → `apply_window_rect`, ver [[LL-097]]). Es el mismo camino de recuperación ya probado del wake-reload post-suspensión ([[LL-095]]). El comando `restart_app` queda en Rust sin consumidores JS.
- **Prevención:** En Tauri, **no usar `app.restart()` para aplicar settings** si el mismo efecto se logra con un reload del WebView — el restart rompe el entorno dev y el reload no. Si de verdad se necesita reiniciar el proceso, asumir que en dev la sesión de `tauri dev` muere con él. Ante un sidebar que muestra "unos textos" en vez de UI, leer el texto: si es la página de error del WebView con `localhost`/`ERR_CONNECTION_REFUSED`, el frontend nunca cargó — es un problema de dev server / URL, no de la feature que se estaba probando.
- **Fuente:** sesión de fix 2026-07-18 (feature "ubicación configurable de la barra"; ver `BACKLOG.md` → "3 Últimas tareas realizadas").
- **Tags:** tauri, tauri-dev, app-restart, vite, err-connection-refused, webview2, reload, appbar, desktop, dev-only

### LL-100 — Input de búsqueda desmontado durante `isLoading` pierde el foco en cada tecleo (Desktop)
- **Síntoma:** En el buscador del panel de Notas del Desktop, al escribir las primeras letras el panel se refresca mostrando resultados pero el cursor desaparece del campo — hay que volver a clickearlo para seguir escribiendo. El buscador general (SearchPanel) no lo sufre.
- **Causa raíz:** El bloque del buscador estaba gateado con `{!isLoading && !error && (…) && (<input …>)}`. Cada cambio del término debounced re-dispara `fetchNotes` → `setIsLoading(true)` → React **desmonta** el `<input>` (se muestra el skeleton) y al terminar lo **remonta como elemento nuevo**, sin foco. El SearchPanel no falla porque su input vive fuera de todo condicional de carga.
- **Solución:** Incluir `isLoading` en la condición de visibilidad del buscador para que nunca se desmonte durante un refetch: `{!error && (isLoading || headerCount > 0 || hasActiveFilters) && (…)}` (`NotesPanel.tsx`). El skeleton sigue reemplazando solo la lista, no la barra de búsqueda.
- **Prevención:** Un control interactivo con el que el usuario está interactuando (inputs de búsqueda/filtros) **no debe compartir el condicional de render con el estado de carga que él mismo dispara**. Al agregar search/filtros server-side a un panel, mantener la barra montada durante `isLoading`. El mismo patrón existía en `BookmarksPanel`, `ContactsPanel`, `SnippetsPanel` y `TasksPanel` — corregidos en los 5 paneles el 2026-07-19 tras confirmarse el síntoma también en Snippets/Contactos.
- **Fuente:** reporte de usuario 2026-07-18 (sesión mejoras barra de control del Desktop).
